...
//		info.setStringPermissions(permissions);
		Set<UPermission> permissions = permissionService.findPermissionByUserId(userId);
		Set<String> set = new HashSet<>();
		for (UPermission upermission : permissions){
			set.add(upermission.getUrl());
		}

		//这里本来不需要这个功能  但是界面上有标签所以要了
		info.setStringPermissions(set);
		TokenManager.setVal2Session("permSet",permissions);	
        return info;  
    }  
    /**
     * 清空当前用户权限信息
